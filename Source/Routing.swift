...
public class Routing: RouteOwner {
    ...
    private convenience init(routes: [Route]) {
        self.init()
        self.routes = routes
    }
    ...
    private func process(searchPath: String,
        ...
        let proxyHandlers: [(Route, (ProxyHandler))] = Array(zipped)
            .flatMap {
                if case .Proxy(let proxyHandler) = $0.1 {
                    return ($0.0, proxyHandler)
                }
                
                return nil
        }
        
        func foreachProxyHandlers(handlers: [(Route, (ProxyHandler))], finished: (() -> Void)) {
            guard let elem = handlers.first else {
                finished()
                return
            }
            
            elem.1(searchPath, parameters, data) { (proxiedPath, proxiedParameters, proxiedData) in
                ...
                foreachProxyHandlers(Array(handlers.dropFirst()), finished: finished)
            }
            
        }
        
        foreachProxyHandlers(proxyHandlers) { [unowned self] in
            var parameters = parameters
            ...
            if let newSearchPath = newSearchPath {
                ...
                for case let (proxiedRoute, .Route(_)) in zipped where proxiedRoute.matches(searchPath) {
